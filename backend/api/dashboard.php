...
try {
    // Kết nối database - KIỂM TRA PORT
    $host = "localhost";
    $port = "3307"; // Hoặc 3307 nếu XAMPP dùng port khác
    $dbname = "nhathuocgb";
    $username = "root";
    $password = "";
    
    // Thêm port vào connection string
    $db = new PDO("mysql:host=$host;port=$port;dbname=$dbname;charset=utf8mb4", $username, $password);
    $db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    
    // Thống kê sản phẩm - BỎ is_active
    ...
    // Top 5 sản phẩm - SỬA order_id thành id
    $stmt = $db->query("
        SELECT 
            p.id,
            p.name,
